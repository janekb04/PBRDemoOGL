...
{	
	...
	vec3 lighting = a_ambient * ao * albedo;
	for (int i = 0; i < a_point_lights.length(); ++i)
	{
		lighting += calculate_point_lighting(a_point_lights[i], normal, v_pos, view_dir, albedo, metallic, roughness);
	}
	for (int i = 0; i < a_directional_lights.length(); ++i)
	{
		lighting += calculate_directional_lighting(a_directional_lights[i], normal, view_dir, albedo, metallic, roughness);
	}
	for (int i = 0; i < a_spotlights.length(); ++i)
	{
		lighting += calculate_spotlight_lighting(a_spotlights[i], normal, v_pos, view_dir, albedo, metallic, roughness);
	}

    f_color = vec4(tonemapper(lighting), alpha);
}