...
namespace ImGui
{
    ...
    IMGUI_API void          NewFrame();                                 // init a new Dear ImGui frame, you can submit any command from this point until Render()/EndFrame().
    ...
    // Windows
    // - Begin() = push window to the stack and init appending to it. End() = pop window from the stack.
    ...
    IMGUI_API bool          BeginPopup(const char* str_id, ImGuiWindowFlags flags = 0);                                             // return true if the popup is open, and you can init outputting to it. only call EndPopup() if BeginPopup() returns true!
    ...
    IMGUI_API void          LogToTTY(int auto_open_depth = -1);                                 // init logging to tty (stdout)
    IMGUI_API void          LogToFile(int auto_open_depth = -1, const char* filename = NULL);   // init logging to file
    IMGUI_API void          LogToClipboard(int auto_open_depth = -1);                           // init logging to OS clipboard
