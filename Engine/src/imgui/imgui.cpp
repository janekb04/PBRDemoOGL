...
/*
 ...
 EXHIBIT 1: USING THE EXAMPLE BINDINGS (imgui_impl_XXX.cpp files from the examples/ folder).
     ...
     while (true)
     {
         // Feed inputs to dear imgui, init new frame
...
void ImGui::UpdateMouseMovingWindowEndFrame()
{
    ...
    // Click on void to focus window and init moving 
...
void ImGui::NewFrame()
{
    ...
    // [DEBUG] Item picker tool - init with DebugStartItemPicker() - useful to visually select an item and break into its call-stack.
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        // Apply focus (we need to call FocusWindow() AFTER setting DC.CursorStartPos so our initial navigation reference rectangle can init around there)
        if (want_focus)
        {
            FocusWindow(window);
...
// Use 'cond' to choose to submit payload on drag init or every frame
